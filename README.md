...
| 폴더 | 버전 | 훅 | 목적 |
| --- | --- | --- | --- |
| [`speech-core`](speech-core/) | 0.1.7 | `pre_llm_call` | 설치된 스킬 본문 셋을 넣고, 한글 말투 수정을 덧붙입니다. |
...
첫 턴에는 본문 전체를 넣습니다. 그다음 턴부터는 스킬마다 한 줄짜리 알림과 한글 말투 수정만 넣습니다. 다만 모델에 실제로 보낸 내용(`api_content`가 있으면 그것, 없으면 `content`)에 본문 전체가 그대로 남아 있지 않으면 본문을 다시 넣습니다. 압축 때문에 본문이 앞부분만 남거나 요약으로 바뀐 경우가 여기에 해당합니다.

사용자가 한 줄로 따로 보낸 명령은 스킬 본문의 규칙대로 반영합니다. `normal mode`는 셋을 모두 끄고, `stop caveman`, `stop adhd mode`, `stop ponytail`은 해당 스킬만 끕니다. `/ponytail lite|full|ultra|off`와 `/caveman <레벨>|off`는 레벨을 바꾸거나 스킬을 끕니다. 문장 안에 섞여 있는 명령은 무시합니다. 꺼진 스킬은 `… MODE OFF` 한 줄로 알립니다. 셋이 모두 꺼지면 한글 말투 수정도 넣지 않습니다. 이 상태는 대화 기록에서만 읽기 때문에, 명령이 담긴 턴이 압축으로 사라지면 기본값으로 돌아갑니다.
